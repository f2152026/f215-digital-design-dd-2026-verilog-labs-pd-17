...
module ripple_adder(
  ...
  FA_Gate FA1 (.a(a[0]),.b(b[0]),.cin(cin),.cout(c1),.sum(sum[0]));
  FA_Gate FA2 (.a(a[1]),.b(b[1]),.cin(c1),.cout(c2),.sum(sum[1]));
  FA_Gate FA3 (.a(a[2]),.b(b[2]),.cin(c2),.cout(c3),.sum(sum[2]));
  FA_Gate FA4 (.a(a[3]),.b(b[3]),.cin(c3),.cout(cout),.sum(sum[3]));
endmodule
